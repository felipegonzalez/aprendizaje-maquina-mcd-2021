...
ajuste_1 <- extract_workflow(resultados, "receta_base_reg_logistica") |> 
  fit(caravan_ent)
preds_logistica <- predict(ajuste_1, testing(caravan_split), type = "prob")
...
preds_kvmc <- predict(ajuste_2, testing(caravan_split), type = "prob") |> 
...
ggplot(dat_calibracion, aes(x = proba_pred, y = prop_obs, 
  ...
  geom_abline() + coord_equal() + geom_rug(data = preds_logistica, aes(x = .pred_Yes))
```
