...
Consideremos un problema donde tenemos unas 100 entradas con 300 casos. Supondremos
...
p_prueba_2 <- predict(mod_reg, dat_pr) %>% 
  bind_cols(dat_pr %>% select(y))
p_prueba_ambas <- bind_rows(p_prueba |> mutate(tipo = "sin penalizar"),
                            p_prueba_2 |> mutate(tipo = "penalizado"))
ggplot(p_prueba_ambas, aes(x = y, y = .pred)) +
  geom_abline(colour = "red") +
  geom_point() + 
  xlab("Predicción") + ylab("y") +
...
res_1 <- metricas(p_prueba, truth = y, estimate = .pred) %>% 
  mutate(tipo = "no penalizado")
res_2 <- metricas(p_prueba_2, truth = y, estimate = .pred) %>% 
...
Y nótese que este modelo solo incluye 4 variables.La traza confirma que la regularización lasso, 
además de encoger 
