...
Entonces, sin requerir supuestos (ver [Distribution-Free Predictive Inference for Regression](https://www.stat.cmu.edu/~ryantibs/papers/conformal.pdf)), tenemos que 
$P(\mathbf{y} \in C(\mathbf{x}))\geq 0.90$ ( y de hecho, si la muestra de prueba
no es muy chica, la cobertura real es aproximadamente de 90\%)

Esta cobertura es robusta, sin embargo, se requieren algunos elementos adicionales
...
construir intervalos predictivos como sigue:
...
de predicción. Podemos hacer también una tabla simple como la que sigue:
...
Y vemos que aunque nuestros intervalos son del 95%, tienen baja cobertura para
...
la subestiman para valores bajos de peso.
...
preds_tbl |> 
  ...
  gt() |> fmt_number(where(is_double), decimals = 2)
```
Y vemos que las coberturas son demasiado altas para valores bajos de la predicción
y tienden a ser bajas para valores altos de la predicción
...
Y esta construcción da intervalos que a distintos niveles de predicción
tiene cobertura condicional cercana al 90%
