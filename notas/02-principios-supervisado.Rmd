...
la regla de "cada año de escolaridad aumenta ingresos potenciales en 100 unidades", 
...
minimizar el error sobre los datos disponibles de entrenamiento.
...
de prueba). Nótese que a diferencia de
...
- No hay brecha grande entre el error de entrenamiento y el error predictivo
...
irredicuble). Pero la razón por la ese error reducible es grande es diferente en cada
caso. Para explicar la diferencia, podemos considerar $f_{lim}$, el predictor que obtendríamos
...
por sesgo
- En ambos casos, una buena parte del error es irreducible. Necesitaríamos otras
variables de entrada para reducir este error.


- El error por sesgo se reduce usando métodos más flexibles o menos
restringidos que puedan capturar patrones claros en los datos. Este término
no tiene qué ver con la muestra de entrenamiento.
...
Y vemos cómo inmediatamente redujimos el error de predicción: en este caso, aunque el
sobreajuste aumentó un poco, la reducción en el sesgo y en el error irreducible.
Examina el caso de vecinos más cercanos.
...
3. Reducir sesgo usando métodos más flexibles o con la estructura apropiada.

Generalmente 2 y 3 están en contraposición.
```
